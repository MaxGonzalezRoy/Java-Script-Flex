...
//Se integra el trabajo realizado en la pre-entrega I con lo solicitado en la pre-entrega II

//Se crea el formulario en el DOM
function crarFormularioRegistro(){
    const formularioDiv = document.getElementById('formulario')
}
    //Se crea el formulario donde almacenar los campos
    const formulario = document.createElement('form')
    formulario.id = 'formularioRegistro'

    const campos = [
        {label: 'Nobre', Id: 'name', type: 'text' },
        {label: 'Correo Electrónico', id: 'email', type: 'email'},
        {label: 'Teléfono', id: 'phone', type: 'text'},
        {label: 'Contraseña', id: 'password', type: 'password'},
        {label: 'Confirmar Contraseña', id: 'comfirPassword', type:'password'}
    ]

    //Se utiliza el metodo forEach para recorrer los campos del array
    campos.forEach(campo => {
        //Se crea la etiqueta label
        const label = document.createElement('label')
        label.textContent = campo.label
        label.setAttribute('for', campo.id)
        
        //Se crea el campo de entrada imput
        const imput = document.createElement('input')
        input.type = campo.type
        input.id = campo.id
        input.required = true

        //Se añaden las etiquetas el formulario
